...
# Overview
To have a working qsfs filesystem there are multiple components that need to work together to make it work. There components are
...
What happens is sice zdb is `append-only` database means the local db will just keep growing linearly with each write (and delete) operation. ZDB will then create db segment files that are granted to **not** change in the future. What happens once a segment file is close (it hit it's max file size) a hook is triggered which in return will trigger `0-stor` to chunk and upload this file to the remote locations (zdbs).
...
If the filesystem then is trying to access a piece of old data, it will make a read call to the local `zdb` if the zdb is then trying to access an old segment of the db that is no longer on disk, another hook is triggered to `0-stor` to download that segment. 0-stor then will re-download the required segment back from the remote locations and re-build it.

Once the zdb segment file is restored, the read operation will continue.

## Docker image
This leads is to the `Dockerfile`. By inspecting the file we can immediately see that the `qsfs` image simply download all the needed components and put them on the image (0-db-fs, 0-db, and 0-stor) it also download an extra components that is not explained above `zinit`.
